...
if (!gatewayToken && !(gatewayKey && gatewaySecret)) {
  console.error('GatewayAPI credentials missing');
}
...
serve(async (req) => {
  try {
    if (!gatewayToken && !(gatewayKey && gatewaySecret)) {
      return new Response(JSON.stringify({ error: 'SMS credentials not configured' }), { status: 500 });
    }
