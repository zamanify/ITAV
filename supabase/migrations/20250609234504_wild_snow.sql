/*
  ...
  This migration fixes recursive RLS policies by:
  1. Creating helper functions to avoid cross-table queries in policies
  2. Dropping all existing problematic policies
  3. Recreating policies using the helper functions to prevent recursion

  ## Changes Made
  - Added helper functions: is_request_owner() and is_direct_recipient()
  - Recreated all RLS policies for requests, request_villagers, and request_groups
  - Eliminated recursive policy dependencies
*/
...
DROP POLICY IF EXISTS "All policies for requests" ON requests;

DROP POLICY IF EXISTS "Users can view request villagers for accessible requests" ON request_villagers;
DROP POLICY IF EXISTS "Requesters and recipients view request villagers" ON request_villagers;
...
DROP POLICY IF EXISTS "All policies for request_villagers" ON request_villagers;
...
DROP POLICY IF EXISTS "All policies for request_groups" ON request_groups;

-- Recreate requests policies without recursion
...
CREATE POLICY "Users can view group requests"
  ...
  USING (
    EXISTS (
      ...
      WHERE rg.request_id = requests.id AND gm.user_id = auth.uid()
    )
    AND NOT EXISTS (
      SELECT 1 FROM user_blocks ub
      WHERE (ub.blocker_id = auth.uid() AND ub.blocked_id = requester_id)
...
-- Recreate request_villagers policies using helper functions
...
-- Recreate request_groups policies using helper functions
